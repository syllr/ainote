...
所有匹配的配置**都会加载到上下文中**，不是后加载的完全替换先加载的。加载顺序为：

```
用户全局 (~/.claude/) → 项目根目录 → 上级目录 → 当前文件所在目录
```

如果规则冲突，**越靠近你正在编辑的文件，优先级越高**。因为后加载的规则在 context 中位置更靠后，模型更容易记住。简单说就是：

- 你的个人全局规则对**所有项目**生效
- 项目规则对**当前项目**生效，可以补充或覆盖你的个人全局规则
- 子目录规则只对**当前子目录**生效，可以补充或覆盖上层规则
...
**优先级**：所有配置都会加载，**越靠近当前工作目录优先级越高**。加载顺序：`用户全局 ~/.claude/ → 项目根目录 → 上级目录 → 当前子目录`，后加载的规则在冲突时更容易被模型优先考虑。
