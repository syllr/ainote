...
### 4. 两种 Skill 设计模式：Composition vs Then

在社区实践中，根据 Skill 封装工作流的方式，总结出两种设计模式：

| 模式       | **Composition Skill（组合式）**                             | **Then Skill（链式/管道式）**                             |
|----------|--------------------------------------------------------|----------------------------------------------------|
| **定义**   | 单个 Skill 内部封装完整的多步骤工作流                                 | 每个 Skill 只做一件单一原子操作                                |
| **职责**   | 自己内部控制流程顺序和分支                                          | 只输出结果，交给下一个 Skill                                  |
| **调用方式** | 用户一次调用完成                                               | 用户手动链式调用：`A → B → C`                               |
| **灵活性**  | 固定流程，使用简单                                              | 灵活组合，适应多种场景                                        |
| **典型例子** | skill-creator（一站式完成 Skill 创建测试优化）、pr-review（一站式 PR 评审） | file-read（只读文件）、prompt（只优化提示词）、code-format（只格式化代码） |

**选择建议**：

- 如果你的工作流**固定不变**，绝大多数情况都按这个顺序走 → **Composition** 更好，用户用起来方便
- 如果你的工作流**需要灵活组合**，不同场景顺序不一样 → **Then** 更好，每个 Skill 独立可以随意组合
