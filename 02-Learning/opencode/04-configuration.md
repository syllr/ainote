...
你可以将配置放置在不同的位置，它们具有不同的优先级顺序。

> 配置文件是**合并在一起**的，而不是替换。配置文件是合并在一起的，而不是被替换。来自以下配置位置的设置会被合并。后面的配置仅在键冲突时覆盖前面的配置。所有配置中的非冲突设置都会被保留。
...
你可以通过 `provider`、`model` 和 `small_model` 选项在 OpenCode 配置中设置要使用的提供商和模型。
...
你还可以配置[本地模型](/docs/models#local)。[了解更多](/docs/models)。
...
- `endpoint` - VPC 端点的自定义端点 URL。这是通用 `baseURL` 选项使用 AWS 特有术语的别名。如果两者都指定，`endpoint` 优先。
...
你可以通过 OpenCode 配置中的 `theme` 选项设置要使用的主题。
...
你还可以使用 `~/.config/opencode/agents/` 或 `.opencode/agents/` 中的 Markdown 文件定义代理。[在此了解更多](/docs/agents)。

### 默认代理

你可以使用 `default_agent` 选项设置默认代理。当未明确指定代理时，将使用该默认代理。
...
默认情况下，分享设置为手动模式，你需要使用 `/share` 命令显式分享会话。
...
你还可以使用 `~/.config/opencode/commands/` 或 `.opencode/commands/` 中的 Markdown 文件定义命令。[在此了解更多](/docs/commands)。
...
OpenCode 启动时会自动下载新版本。你可以使用 `autoupdate` 选项禁用此功能。
...
如果你不想自动更新但希望在新版本可用时收到通知，可将 `autoupdate` 设置为 `"notify"`。 请注意，此功能仅在未通过 Homebrew 等包管理器安装时有效。
...
将插件文件放置在 `.opencode/plugins/` 或 `~/.config/opencode/plugins/` 中。你还可以通过 `plugin` 选项从 npm 加载插件。
...
你可以通过 `instructions` 选项为所使用的模型配置指令。
...
你可以通过 `disabled_providers` 选项禁用自动加载的提供商。当你希望阻止某些提供商被加载（即使其凭据可用）时，此选项非常有用。
...
你可以通过 `enabled_providers` 选项指定允许使用的提供商白名单。设置后，仅启用指定的提供商，所有其他提供商将被忽略。
...
当你希望限制 OpenCode 仅使用特定提供商，而不是逐一禁用其他提供商时，此选项非常有用。
...
> 警告：实验性选项不稳定。它们可能会在不另行通知的情况下被更改或移除。
