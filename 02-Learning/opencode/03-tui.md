...
> 所有文件更改也会被恢复。在内部，这使用 Git 来管理文件更改。因此您的项目**需要是一个 Git 仓库**。
...
> 所做的任何文件更改也会被还原。在内部，这使用 Git 来管理文件更改。因此您的项目**需要是一个 Git 仓库**。
...
> 某些编辑器（如 VS Code）需要以 `--wait` 标志启动。某些编辑器需要命令行参数才能以阻塞模式运行。`--wait` 标志使编辑器进程阻塞直到关闭。
